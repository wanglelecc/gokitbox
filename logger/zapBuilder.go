...
func (log *zapBuilder) LoadConfig(config *Config) {
	...
	default:
		fmt.Fprintf(os.Stderr, "LoadConfig: Error: Required child <%s> for filter has unknown value %s\n", "level", config.Level)
		os.Exit(1)
	}
	...
	if config.Console == true {
		core = zapcore.NewTee(core, zapcore.NewCore(encoder, os.Stdout, lvl))
	}
...
func (log *zapBuilder) LoggerF(ctx context.Context, lvl string, tag string, message string, fields ...interface{}) {
	...
	log.LoggerW(ctx, true, lvl, tag, message, fields...)
}

func (log *zapBuilder) LoggerW(ctx context.Context, mod bool, lvl string, tag string, message string, fields ...interface{}) {
	if tag == "" {
		tag = "NoTagError"
	}
	...
	if mod {
		...
		fields = []interface{}{"x_tag", tag}
	}

	fields = append(log.Build(ctx), fields...)
